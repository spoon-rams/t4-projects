...
try {
 ...
 if (! empty($config['option'])) {
  ...
  foreach ($variables as $variable) {
   ...
   if (isset($output_array[1], $output_array[2])) {
    $key   = trim(str_replace(['"', '\''], '', $output_array[1]));
    $value = str_replace(['"', '\''], '', trim($output_array[2]));
 ...
 $stopWords = ['/\band\b/is', '/\bof\b/is', '/\bin\b/is', '/\bor\b/is', '/\bwith\b/is', '/\bthe\b/is', '/\bat\b/is'];
 ...
 $queryHandler->setDontRemoveStopwords(['residenceName', 'residenceCampus', 'residenceClass']);
 $queryHandler->setDontTokenize(['residenceName', 'residenceCampus', 'residenceClass']);
 ...
 if ($queryHandler->isQuerySet('keywords')) {
  $substringSearch->setMember('element', 'residenceName');
  ...
  $substringSearch->setMember('element', 'residenceName');
  $substringSearch->runFilter();
  $substringSearch->setMember('element', 'residenceClass');
  $substringSearch->runFilter();
  $substringSearch->setMember('element', 'residenceCampus');
  $substringSearch->runFilter();
  $substringSearch->setMember('element', 'residenceOccupancy');
  $substringSearch->runFilter();
  $substringSearch->setMember('element', 'residenceType');
  $substringSearch->runFilter();
  $search->combineResults();
 }

 if ($queryHandler->isQuerySet('residenceCampus')) {
  $exactSearch->setMember('element', 'residenceCampus');
  $exactSearch->setMember('query', $queryHandler->getQueryValue('residenceCampus'));
  $exactSearch->setMember('multipleValueState', true);
  $exactSearch->setMember('multipleValueSeparator', ', ');
  $exactSearch->runFilter();
 }

 if ($queryHandler->isQuerySet('residenceOccupancy')) {
  $exactSearch->setMember('element', 'residenceOccupancy');
  $exactSearch->setMember('query', $queryHandler->getQueryValue('residenceOccupancy'));
  $exactSearch->setMember('multipleValueState', true);
  $exactSearch->setMember('multipleValueSeparator', ', ');
  $exactSearch->runFilter();
 }

 if ($queryHandler->isQuerySet('residenceClass')) {
  $exactSearch->setMember('element', 'residenceClass');
  $exactSearch->setMember('query', $queryHandler->getQueryValue('residenceClass'));
  $exactSearch->setMember('multipleValueState', true);
  $exactSearch->setMember('multipleValueSeparator', ', ');
  $exactSearch->runFilter();
 }

 if ($queryHandler->isQuerySet('residenceLiving')) {
  $exactSearch->setMember('element', 'residenceLiving');
  $exactSearch->setMember('query', $queryHandler->getQueryValue('residenceLiving'));
  $exactSearch->setMember('multipleValueState', true);
  $exactSearch->setMember('multipleValueSeparator', ', ');
  $exactSearch->runFilter();
 }

 if ($queryHandler->isQuerySet('residenceType')) {
  $exactSearch->setMember('element', 'residenceType');
  $exactSearch->setMember('query', $queryHandler->getQueryValue('residenceType'));
  $exactSearch->setMember('multipleValueState', true);
  $exactSearch->setMember('multipleValueSeparator', ', ');
  $exactSearch->runFilter();
 }
